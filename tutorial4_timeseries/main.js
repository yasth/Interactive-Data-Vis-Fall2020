...
d3.csv("data.csv", d => {
  ...
  ratio: +d.Ratio,
  urban: +Math.floor(d.Population*(d.Ratio/100)),
  rural: +Math.floor(d.Population*(1-(d.Ratio/100))),
}}).then(raw_data => {
...
function init() {
  ...
  selectElement
    .selectAll("option")
    .data([
      ...Array.from(new Set(state.data.map(d => d.ratio))),
      default_selection,
    ])
    .join("option")
    .attr("value", d => d)
    ...
    .call(
      selection =>
        selection
          ...
          .attr("cy", d => yScale(d.ratio)) // started from the bottom, now we're here
    );
  let lineData = filteredData.map(d => new Array(d.year,d.ratio))
  console.log(lineData)
  const line = svg.append("path") .attr("fill", "none")
  .attr("stroke", "steelblue")
  .attr("stroke-width", 1.5)
    .datum(filteredData).attr("d", d => lineFunc(d))
    //   enter =>
    //     enter
    //       .attr("opacity", 1) // start them off as opacity 0 and fade them in
